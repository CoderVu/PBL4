...
public class Client extends javax.swing.JFrame {
    ...
    public Client(){
        ...
        try {
          database = new DatabaseManager("team_speakkk","root","123456789");
    ...
    public void isLogged(String username){
        try {
            ...
            clientHandler.newReading();
            clientImage = new ClientImage(this);
            //clientImage.newReading();
    ...
    public void initUser(String username) throws SQLException{
        ...
        user.next();
        database.user = new User(user.getInt("id"), user.getString("username"),user.getString("password"),user.getString("mail"));  
    }
    ...
    private void menuLoginMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_menuLoginMouseClicked
        if (!login.isVisible()) {
            login.setBounds(0, 0, login.getWidth(), login.getHeight());
            this.desktop.add(login, JLayeredPane.DEFAULT_LAYER, 0); // Sử dụng layer mặc định và vị trí 0
    ...
    private void menuRegisterMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_menuRegisterMouseClicked
       if(!register.isVisible()){
           register.setBounds(0, 0, register.getWidth(), register.getHeight());
           this.desktop.add(register, JLayeredPane.DEFAULT_LAYER, 0); // Sử dụng layer mặc định và vị trí 0
    ...
    private void menuRoomsMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_menuRoomsMouseClicked
        if (!rooms.isVisible()) {
            rooms.setBounds(0, 0, 400, 300);

            // Check if the frame is not added, then add it
            if (!Arrays.asList(desktop.getAllFrames()).contains(rooms)) {
                desktop.add(rooms);
            }
