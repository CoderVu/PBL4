...
import chatapplication.connection.ClientImage;
...
public class RoomFrame extends javax.swing.JInternalFrame {
     ...
     */
    public RoomFrame(Room room, DatabaseManager database, ClientHandler clientHandler, JDesktopPane desktop, ClientImage clientimage) {
    ...
    private JButton getButton(Model_Emoji data) {
        ...
        cmd.addActionListener(new ActionListener() {
            ...
            // Lấy thời gian hiện tại và định dạng nó
                    ...
                    saveMessageToDatabase("", message, currentTime, senderId, userList);
    ...
    private void handleSelectButton() throws Exception {
        File selectedFile = chooseImageFile();
        if (selectedFile != null) {
            try {
                clientImage.sendImageToServer(selectedFile);
        ...
        String messageImage = "C:\\PBL4\\ChatApplication-Copy (1)\\src\\chatapplication\\serverPicture\\receive" + selectedFile.getName();
        int senderId = getUserIdByUsername(database.user.getUsername());
        saveMessageToDatabase("", messageImage, currentTime, senderId, userList);
        try {
            clientHandler.writeMessage("roomimage", database.user.getUsername(), "", messageImage, userList);
        } catch (IOException ex) {
...
//GEN-LAST:event_sendButtonActionPerformed
    ...
    private void saveMessageToDatabase(String message, String messageIcon, String timestamp, int senderId, List<String> userList) {
        try {
            int roomId = getRoomIdByRoomName(room.getRoom());
            ...
            // Lưu tin nhắn cho từng người nhận trong danh sách userList
            for (String receiverUsername : userList) {
    ...
    public void loadChatHistory(String roomName, int currentUserId) {
        try {
            Set<Integer> processedReceivers = new HashSet<>(); // Sử dụng Set để theo dõi các người nhận đã được xử lý
            String query = "SELECT sender_id, GROUP_CONCAT(DISTINCT receiver_id) as receivers, message_text, timestamp, message_icon FROM chat_messages "
                    + "WHERE chat_room_id = (SELECT id FROM rooms WHERE room_name = ?) "
                    + "GROUP BY sender_id, message_text, timestamp, message_icon "
                    + "ORDER BY timestamp";
            java.sql.PreparedStatement preparedStatement = database.connection.prepareStatement(query);
            preparedStatement.setString(1, roomName);
    ...
    public void receiveMessage(String fromUser, String toUser, String message, String messageIcon, String type) {
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        String currentTime = dateFormat.format(new Date());
        ...
        if (fromUser.equalsIgnoreCase(database.user.getUsername())) {
            ...
            if (!messageIcon.isEmpty()){
                if ("chat".equals(type)){
                    ...
                    chat.addItemRight("[" + currentTime + "] " + fromUser + ": " + message, new ImageIcon(messageIcon));
                }
            ...
            if (!messageIcon.isEmpty()){
                if ("chat".equals(type)){
                    ...
                    chat.addItemLeft("[" + currentTime + "] " + fromUser + ": " + message, new ImageIcon(messageIcon));
                }
            }else{
